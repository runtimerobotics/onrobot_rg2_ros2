# my_python_package/rg2_driver.py
...
from std_msgs.msg import Int32  # Change to Int32 message type
...
class RG2DriverNode(Node):
    def __init__(self):
        super().__init__('rg2_driver_node')

        # Declare parameters
        self.declare_parameter('toolchanger_ip', '192.168.1.48')
        self.declare_parameter('gripper_type', 'rg2')
        self.declare_parameter('toolchanger_port', 502)

        # Get parameters
        self.toolchanger_ip = self.get_parameter('toolchanger_ip').get_parameter_value().string_value
        self.gripper = self.get_parameter('gripper_type').get_parameter_value().string_value
        self.toolchanger_port = self.get_parameter('toolchanger_port').value

        self.rg = RG(self.gripper, self.toolchanger_ip, self.toolchanger_port)

        # Create subscriber for gripper commands using Int32
        ...
        self.get_logger().info('RG2 Driver Node has been started.')

    def gripper_callback(self, msg):
        """Callback function for gripper commands"""
        try:
            width_val = msg.data  # Use integer value directly (in tenths of mm)
            self.rg.move_gripper(width_val)
            self.get_logger().info(f'Moving gripper to width: {width_val/10.0} mm')
...
def main(args=None):
    ...
    # Do other initialization here if necessary
